...
import { MaterialsLibraryForm } from '@/components/materials-library-form'
import { BasicTierUpsellCard } from '@/components/basic-tier-upsell-card'
import { User, CreditCard, Factory, Percent, Cylinder, Cpu, ChevronRight, Sparkles } from 'lucide-react'

export default function CompanySettingsPage() {
  const user = useCostingStore(s => s.user)
  const isBasicTier = user?.tier !== 'Pro'

  const [activeTab, setActiveTab] = useState<'profile' | 'billing' | 'workcenters' | 'materials' | 'overheads' | 'commercials' | 'allowances'>('materials')
      ...
      <nav className="w-64 bg-white border-r border-slate-200 p-5 flex flex-col shrink-0">
        ...
        <div className="mt-auto p-4 bg-indigo-50/60 rounded-lg border border-indigo-100 space-y-1">
          ...
          <p className="text-xs text-slate-600">
            Material cutting speeds (Vc) and feeds (f) automatically calculate turning RPM and machine cycle times.
          </p>
        </div>
      ...
      <div className="flex-1 p-8 overflow-y-auto relative">
        {activeTab === 'materials' && (
          <div className="relative">
            <MaterialsLibraryForm isDisabled={isBasicTier} />
