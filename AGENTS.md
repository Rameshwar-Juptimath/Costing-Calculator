...
| **Material Machinability & Speeds/Feeds** | [backend/app/models/material_machinability.py](file:///d:/Work/My_Work/freelance_projects/Costing_calculator/backend/app/models/material_machinability.py), [backend/app/schemas/material.py](file:///d:/Work/My_Work/freelance_projects/Costing_calculator/backend/app/schemas/material.py), [backend/app/services/machining_service.py](file:///d:/Work/My_Work/freelance_projects/Costing_calculator/backend/app/services/machining_service.py), [backend/app/api/v1/materials.py](file:///d:/Work/My_Work/freelance_projects/Costing_calculator/backend/app/api/v1/materials.py), [frontend/components/materials-library-form.tsx](file:///d:/Work/My_Work/freelance_projects/Costing_calculator/frontend/components/materials-library-form.tsx) |
...
## 7. Mandatory Protocol for Maintaining `AGENTS.md`, `README.md`, and the Upgrade Page

> ⚠️ **CRITICAL INSTRUCTION FOR ALL AI AGENTS**:
> Whenever you modify the application (adding files, deleting files, renaming routes, altering state stores, updating schemas, adding feature endpoints, changing test setups, or adding/removing capabilities), you **MUST UPDATE `AGENTS.md`, `README.md`, AND THE UPGRADE PAGE (`/dashboard/upgrade`) IN THE EXACT SAME TASK STEP**.
...
### Rules for Feature Synchronization on Upgrade Page (`http://localhost:3030/dashboard/upgrade`):
- **Feature Additions**: Whenever a new feature or capability is added to the application, it **MUST be added to the Pro/Upgrade features showcase** on the upgrade page ([frontend/app/dashboard/upgrade/page.tsx](file:///d:/Work/My_Work/freelance_projects/Costing_calculator/frontend/app/dashboard/upgrade/page.tsx) / `http://localhost:3030/dashboard/upgrade`). Ensure the feature card and description reflect the new capability (e.g. Material Library, Work Centers, Automated Overheads, Tax & Commercials, Export Reports, Extended Archive).
- **Feature Removals / Deprecations**: Whenever a feature is removed or deprecated from the application, it **MUST be removed from the upgrade page** ([frontend/app/dashboard/upgrade/page.tsx](file:///d:/Work/My_Work/freelance_projects/Costing_calculator/frontend/app/dashboard/upgrade/page.tsx)) immediately.
- **Consistency Guarantee**: The features displayed on `http://localhost:3030/dashboard/upgrade` must always stay 100% synchronized with the live platform capabilities.

*Keeping `AGENTS.md`, `README.md`, and the Upgrade Page strictly synchronized guarantees zero documentation drift, zero token waste, and perfect codebase awareness for human developers and AI agents alike!*
